...
async function checkRecentConsumos() {
  try {
    ...
    consumos.forEach((consumo, index) => {
      console.log(`${index + 1}. ${consumo.registeredAt.toISOString()} - Cliente: ${consumo.cliente.nombre} (${consumo.cliente.cedula}) - Total: $${consumo.total} - Empleado: ${consumo.empleado.name || 'N/A'} - Tipo: ${consumo.ocrText?.startsWith('MANUAL:') ? 'MANUAL' : 'OCR'}`);
    });
