...
const TARGET_FILES = [
  ...
  {
    path: 'src/app/superadmin/SuperAdminDashboard.tsx',
    outputDir: 'src/components/superadmin',
  },
...
async function runComponentExtraction() {
  ...
  for (const target of TARGET_FILES) {
    ...
    try {
      ...
      console.log(
        `✅ Componentes extraídos: ${extractedComponents.join(', ')}`
      );

      // Actualizar archivo original
      const relativeImportPath = path
        .relative(path.dirname(absolutePath), absoluteOutputDir)
        .replace(/\\/g, '/');
