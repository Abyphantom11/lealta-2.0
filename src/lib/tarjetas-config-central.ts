...
export async function getTarjetasConfigCentral(businessId: string): Promise<ConfiguracionCentralTarjetas> {
  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      // 🔧 TRANSFORMAR NIVELES DEL JSON A ESTRUCTURA CENTRAL
      if (config.tarjetas && config.tarjetas[0] && config.tarjetas[0].niveles) {
        ...
        console.log(`✅ [CENTRAL] Transformados ${tarjetas.length} niveles desde JSON`);
      } else {
