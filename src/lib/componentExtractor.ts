 ...
 */
export async function extractComponents(options: ExtractComponentOptions): Promise<string[]> {
  const { 
    sourcePath, 
    outputDir, 
    minLines = 50,
  ...
  traverse(ast, {
    FunctionDeclaration(path: any) {
      if (shouldExtractComponent(path.node, minLines, mode, componentNames)) {
        ...
        const componentFilePath = getComponentFilePath(outputDir, componentName);
    ...
    VariableDeclaration(path: any) {
      ...
      path.node.declarations.forEach((declaration: any) => {
        if (
          ...
          if (shouldExtractComponent(declaration, minLines, mode, componentNames)) {
            const componentCode = generate(path.node).code;
...
function shouldExtractComponent(
  ...
  if (mode === 'manual') {
    const nodeName = node.id?.name || (t.isVariableDeclarator(node) && t.isIdentifier(node.id) ? node.id.name : '');
 ...
 */
function getComponentFilePath(outputDir: string, componentName: string): string {
 ...
 */
function createComponentFile(filePath: string, componentCode: string, sourceCode: string) {
 ...
 */
function extractRelevantImports(sourceCode: string, componentCode: string): string {
 ...
 */
export function updateSourceFile(sourcePath: string, extractedComponents: string[], relativePath: string): void {
