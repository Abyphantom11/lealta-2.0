          ...
          <div className="p-6 bg-white rounded-lg shadow-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <div className="bg-blue-50 p-4 rounded-lg shadow-sm">
                <h3 className="text-lg font-semibold text-blue-700">Total Clientes</h3>
                <p className="text-3xl font-bold">{statsContext.stats?.totalClients || 0}</p>
              </div>
              <div className="bg-green-50 p-4 rounded-lg shadow-sm">
                <h3 className="text-lg font-semibold text-green-700">Ingresos Totales</h3>
                <p className="text-3xl font-bold">${statsContext.stats?.totalRevenue?.toFixed(2) || '0.00'}</p>
              </div>
              <div className="bg-yellow-50 p-4 rounded-lg shadow-sm">
                <h3 className="text-lg font-semibold text-yellow-700">Consumos</h3>
                <p className="text-3xl font-bold">{statsContext.stats?.totalConsumos || 0}</p>
              </div>
              <div className="bg-red-50 p-4 rounded-lg shadow-sm">
                <h3 className="text-lg font-semibold text-red-700">Pendientes de Pago</h3>
                <p className="text-3xl font-bold">{statsContext.stats?.unpaidCount || 0}</p>
              </div>
            </div>
              ...
              <ul className="space-y-2">
                ...
                <li>
                  <button
                    onClick={() => setCurrentTab('menu')}
                    className={`w-full text-left px-4 py-2 rounded transition-colors ${
                      currentTab === 'menu' ? 'bg-blue-600' : 'hover:bg-gray-700'
                    }`}
                  >
                    Gestión de Menú
                  </button>
                ...
                <li>
                  <button
                    onClick={() => setCurrentTab('clientes')}
                    className={`w-full text-left px-4 py-2 rounded transition-colors ${
                      currentTab === 'clientes' ? 'bg-blue-600' : 'hover:bg-gray-700'
                    }`}
                  >
                    Gestión de Clientes
                  </button>
                ...
                <li>
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 rounded transition-colors ${
                      currentTab === 'tarjetas' ? 'bg-blue-600' : 'hover:bg-gray-700'
                    }`}
                  >
                ...
                <li>
                  <button
                    onClick={() => setCurrentTab('estadisticas')}
                    className={`w-full text-left px-4 py-2 rounded transition-colors ${
                      currentTab === 'estadisticas' ? 'bg-blue-600' : 'hover:bg-gray-700'
                    }`}
                  >
                    Estadísticas
                  </button>
                ...
                <li>
                  <button
                    onClick={() => setCurrentTab('config')}
                    className={`w-full text-left px-4 py-2 rounded transition-colors ${
                      currentTab === 'config' ? 'bg-blue-600' : 'hover:bg-gray-700'
                    }`}
                  >
                    Configuración
                  </button>
