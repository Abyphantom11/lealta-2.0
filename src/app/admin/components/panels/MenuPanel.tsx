...
export const MenuPanel: React.FC<Readonly<MenuPanelProps>> = ({ menuContext }) => {
  const {
    categories,
    products,
    loading,
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {categories.map((categoria: MenuCategory) => (
            <div key={categoria.id} className="border rounded-md p-4 flex justify-between items-center">
              ...
              {products.map((item: MenuItem) => (
                <tr key={item.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {categories.find(cat => cat.id === item.categoryId)?.nombre || 'Sin categoría'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap capitalize">{item.tipoProducto}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${item.disponible ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
