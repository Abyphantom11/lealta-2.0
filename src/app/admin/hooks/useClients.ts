...
export function useClients() {
  ...
  const fetchClientes = useCallback(async (params: ClienteSearchParams) => {
    ...
    try {
      ...
      if (params.minPuntos)
        queryParams.append('minPuntos', params.minPuntos.toString());
      if (params.maxPuntos)
        queryParams.append('maxPuntos', params.maxPuntos.toString());
      queryParams.append('page', params.page.toString());
      queryParams.append('limit', params.limit.toString());

      const response = await fetch(
        `/api/admin/clientes?${queryParams.toString()}`
      );
  ...
  const getClienteById = useCallback(
    async (cedula: string): Promise<Cliente | null> => {
      setLoading(true);
      setError(null);
      try {
  ...
  const getClienteHistory = useCallback(
    async (cedula: string): Promise<ClientHistoryItem[]> => {
      setLoading(true);
      setError(null);
      try {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          setError(
            errorData.message || 'Error al cargar el historial del cliente'
          );
  ...
  const saveCliente = useCallback(
    async (cliente: ClienteFormData): Promise<ClienteOperationResult> => {
      setLoading(true);
      try {
        // Determinar si es una actualización o creación
        const isUpdate = Boolean(
          cliente.cedula && clientes.some(c => c.cedula === cliente.cedula)
        );
  ...
  const updatePuntos = useCallback(
    async (
      cedula: string,
      puntos: number,
      motivo: string
