...
export function useConfig() {
  ...
  const fetchConfig = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logger.warn('No se encontró configuración, usando valores predeterminados:', errorData);
      }
    } catch (error) {
  ...
  const saveConfig = useCallback(async (newConfig: PortalConfig): Promise<ConfigOperationResult> => {
    setLoading(true);
    try {
  ...
  const updateBanner = useCallback(async (banner: BannerConfig): Promise<ConfigOperationResult> => {
    ...
    try {
      ...
      const bannerIndex = updatedConfig.banners.findIndex(b => b.id === banner.id);
  ...
  const updatePromocion = useCallback(async (promocion: PromocionConfig): Promise<ConfigOperationResult> => {
    if (!config) return { success: false, message: 'No hay configuración cargada' };
    ...
    try {
      ...
      const promocionIndex = updatedConfig.promociones.findIndex(p => p.id === promocion.id);
  ...
  const updateEvento = useCallback(async (evento: EventoConfig): Promise<ConfigOperationResult> => {
    if (!config) return { success: false, message: 'No hay configuración cargada' };
    ...
    try {
      ...
      const eventoIndex = updatedConfig.eventos.findIndex(e => e.id === evento.id);
  ...
  const updateRecompensa = useCallback(async (recompensa: RecompensaConfig): Promise<ConfigOperationResult> => {
    if (!config) return { success: false, message: 'No hay configuración cargada' };
    ...
    try {
      ...
      const recompensaIndex = updatedConfig.recompensas.findIndex(r => r.id === recompensa.id);
  ...
  const updateTarjeta = useCallback(async (tarjeta: TarjetaConfig): Promise<ConfigOperationResult> => {
    if (!config) return { success: false, message: 'No hay configuración cargada' };
    ...
    try {
      ...
      const tarjetaIndex = updatedConfig.tarjetas.findIndex(t => t.id === tarjeta.id);
  ...
  // Función para actualizar el favorito del día
  const updateFavoritoDia = useCallback(async (favorito: FavoritoDelDiaConfig): Promise<ConfigOperationResult> => {
    if (!config) return { success: false, message: 'No hay configuración cargada' };
    ...
    try {
      ...
      // Establecer el favorito del día
      updatedConfig.favoritoDelDia = {
        ...favorito,
        id: favorito.id || `favorito_${Date.now()}`
      };
  ...
  const uploadImage = useCallback(async (file: File, type: string, id?: string): Promise<string> => {
    setLoading(true);
    try {
