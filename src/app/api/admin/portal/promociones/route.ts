...
// GET - Obtener todas las promociones (incluyendo inactivas) para administración
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error obteniendo promociones para admin:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`✅ Promoción creada por admin: ${promocion.id} - ${promocion.title}`);
    return NextResponse.json({ promocion }, { status: 201 });
  } catch (error) {
    console.error('Error creando promoción desde admin:', error);
    return NextResponse.json(
      { error: 'Error interno del servidor' },
      { status: 500 }
    );
...
// PUT - Actualizar promoción existente (admin)
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log(`✅ Promoción actualizada por admin: ${promocion.id} - ${promocion.title}`);
    return NextResponse.json({ promocion });
  } catch (error) {
    console.error('Error actualizando promoción desde admin:', error);
    return NextResponse.json(
      { error: 'Error interno del servidor' },
      { status: 500 }
    );
...
// DELETE - Eliminar promoción (admin)
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(`✅ Promoción eliminada por admin: ${id}`);
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Error eliminando promoción desde admin:', error);
    return NextResponse.json(
      { error: 'Error interno del servidor' },
      { status: 500 }
    );
