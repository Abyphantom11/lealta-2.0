...
export const useAutoRefreshPortalConfig = (options: UseAutoRefreshOptions = {}) => {
  ...
  const fetchConfig = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/portal/config-v2?businessId=${configBusinessId}&timestamp=${Date.now()}`,
        {
      ...
      if (response.ok) {
        ...
        console.log(`✅ Config v2 (PostgreSQL) updated successfully at ${new Date().toLocaleTimeString()}`);
        console.log('🔍 Raw API data:', {
          banners: data.data?.banners?.length || 0,
          promociones: (data.data?.promociones || data.data?.promotions)?.length || 0,
          recompensas: (data.data?.recompensas || data.data?.rewards)?.length || 0,
          favoritoDelDia: (data.data?.favoritoDelDia || data.data?.favorites)?.length || 0,
          businessDay: currentDay
        });
      } else {
        console.error('❌ Error fetching config v2:', response.status);
      }
    } catch (error) {
      console.error('❌ Error fetching portal config v2:', error);
    } finally {
  ...
  const getPromocionesForBusinessDay = useCallback(async () => {
    if (!config?.data?.promociones && !config?.data?.promotions) return [];
  ...
  const getFavoritoDelDia = useCallback(async (diaActual?: string) => {
    const favoritoData = config?.data?.favoritoDelDia || config?.data?.favorites || [];
  ...
  const getRecompensas = useCallback(() => {
    const recompensas = config?.data?.recompensas || config?.data?.rewards || [];
    return recompensas.filter((r: any) => r.activo !== false) || [];
  }, [config]);

  const getBanners = useCallback(() => {
    const banners = config?.data?.banners || [];
