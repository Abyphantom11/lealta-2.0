...
async function runComponentExtraction() {
  try {
    ...
    logger.info(
      `Se extrajeron ${extractedComponents.length} componentes:`,
      extractedComponents
    );
    ...
    if (extractedComponents.length > 0) {
      ...
      logger.success(
        'Archivo original actualizado con las importaciones de componentes'
      );
    }
  } catch (error) {
...
async function extractSpecificComponents() {
  try {
    ...
    logger.info(
      `Se extrajeron ${extractedComponents.length} componentes específicos:`,
      extractedComponents
    );
    ...
    if (extractedComponents.length > 0) {
      ...
      logger.success(
        'Archivo original actualizado con las importaciones de componentes específicos'
      );
    }
  } catch (error) {
