...
async function testNuevoSistemaPuntos() {
  try {
    ...
    const consumoData = {
      ...
      productos: [
        { nombre: 'Café Espresso', cantidad: 2 }
      ],
    ...
    if (result.success) {
      ...
      if (result.data.cliente.puntosNuevos === puntosEsperados) {
        console.log(`✅ Cálculo de puntos correcto: $${consumoData.totalManual} × 2 = ${puntosEsperados} puntos`);
      } else {
        console.log(`❌ Error en cálculo: esperaba ${puntosEsperados}, obtuvo ${result.data.cliente.puntosNuevos}`);
      }
    } else {
